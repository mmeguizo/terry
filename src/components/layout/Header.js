...
const Header = () => {
  ...
  return (
    <header className="fixed w-full top-0 z-50 flex items-stretch">
      ...
      <div className={`fixed top-0 right-0 h-full w-full bg-neutral-900/90 backdrop-blur-xl transform transition-transform duration-600 ease-in-out z-50 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-6">
            <ul className="space-y-2">
              {config.menu.map((item, index) => (
                <li key={index}>
                  <Link href={item.url} onClick={closeMobileMenu} className="block py-3 px-4 rounded-md uppercase font-bold text-md transition-all duration-300 relative overflow-hidden group" style={{ color: config.textColor }}>
      ...
      <div className={`fixed top-0 right-0 h-full w-full bg-neutral-900/90 backdrop-blur-xl transform transition-transform duration-600 ease-in-out z-50 ${isWebsitesMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between ps-8 pe-6 py-6 border-b border-white/20">
